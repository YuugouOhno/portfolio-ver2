...
const Control = () => {
    ...
    return (
        <>
            <div className={`${showControl && how ? "" : "hidden"} fixed bottom-0 left-0 w-auto h-auto z-50 bg-gray-300 dark:bg-gray-700 bg-opacity-70 dark:bg-opacity-70 text-black dark:text-white`}>
                <p className="text-center">遊び方</p>
                <p>①画面上をクリックした状態でマウスを動かすと魚全体を動かせます。</p>
                <p>②トラックパットで画面を拡大/縮小する感じの操作をすると、視点を動かせます。</p>
                <p>③コントロールパネルの数値を操作して遊ぼう。</p>
                <p className="text-center" onClick={()=>setHow(false)}>閉じる</p>
            </div>
